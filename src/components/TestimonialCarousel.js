...
const TestimonialCarousel = ({ slider }) => {
  return (
    ...
    autoPlay={false}
    infiniteLoop={true}
    className='w-full max-w-[1024px] flex item-center justify-center'
    >
      {slider.map((item, index) => {
        // destructure item
        const {image, message, name, occupaation} = item;
        return (
          <div className='max-w-[840px] mx-auto px-24 text-white' key={index}>
            ...
            <div className='font-medium italic '>{occupaation}</div>
          </div>
        )
      })}
    </Carousel>
